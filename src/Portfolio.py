...
class Portfolio:
    ...
    @staticmethod
    def formulate_trade_action_from_signal(coint_pair) -> TradingAction:
    ...
    def update_portfolio_data(self, traded_pair) -> None:
        self._update_pf_committed_capital(traded_pair)
        self._update_pf_free_cash(traded_pair)
        self._update_pf_locked_cash(traded_pair)
    ...
    def _update_pf_committed_capital(self, traded_pair) -> None:
        self.pf_committed_capital += traded_pair.tot_committed_capital

    def _update_pf_free_cash(self, traded_pair) -> None:
        self.pf_free_cash -= traded_pair.tot_committed_capital

    def _update_pf_locked_cash(self, traded_pair) -> None:
        self.pf_locked_cash += traded_pair.locked_cash


    @staticmethod
    def _units_finder(py, hedge_ratio, min_notional=48_000, max_notional=52_000) -> Tuple:
