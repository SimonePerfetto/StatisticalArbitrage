...
class DateManager:
    ...
    def __define_key_dates(self) -> None:
        ...
        self.coint_end_date = self.add_date(self.coint_start_date, self.coint_window - 1)
        self.trade_start_date = self.add_date(self.coint_end_date, 1)
        self.trade_end_date = self.add_date(self.trade_start_date, self.trade_window - 1)
        self.no_new_trades_from_date = self.add_date(self.trade_end_date, -self.no_new_trades_days)
    ...
    def update_key_dates(self) -> None:
        ...
        if days_left >= self.trade_window * 1.5 - 1:  # 1.5 times trading window,
            self.coint_start_date = self.add_date(self.trade_end_date, -self.coint_window + 1)
            self.coint_end_date = self.trade_end_date
            self.trade_start_date = self.add_date(self.trade_end_date, 1)
            self.trade_end_date = self.add_date(self.trade_start_date, self.trade_window - 1)
            self.no_new_trades_from_date = self.add_date(self.trade_end_date, -self.no_new_trades_days)
        else:
