...
class Cointegrator:
        ...
        for cluster in tickers_per_cluster:
            for pair in itertools.combinations(list(cluster), 2):
                ...
                if is_cointegrated:
                    ...
                    # ignore cointegrated pairs for which at least one stock is
                    # already in the current_cointegrated_pairs list
                    if self.__check_pair_member_already_in_list(pair, current_cointegrated_pairs): continue
    ...
    def __logged_lin_reg(self, x: DataFrame, y: DataFrame) -> Tuple[array, float, LinearRegression]:
        log_x, log_y = x, y #np.log(x), np.log(y)
    ...
    def __check_pair_member_already_in_list(self, pair: Tuple, coint_pairs_list):
        stock_1_check = any([pair[0] in coint_p.pair for coint_p in coint_pairs_list])
        stock_2_check = any([pair[1] in coint_p.pair for coint_p in coint_pairs_list])
        return stock_1_check or stock_2_check
    ...
    def get_previous_cointegrated_pairs(self, current_window):
        ...
        for coint_pair in self.previous_cointegrated_pairs:
            ...
            t1_latest_log_price = float(t1_most_recent.values[0][0]) #np.log(float(t1_most_recent.values[0][0]))
            t2_latest_log_price = float(t2_most_recent.values[0][0]) #np.log(float(t2_most_recent.values[0][0]))
